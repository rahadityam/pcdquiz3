...
def convolution(img, kernel):
    # Apply the convolution directly to each channel
    new_img = np.zeros_like(img)

    # Apply the kernel to each channel using scipy.ndimage.convolve
    for channel in range(3):
        new_img[:, :, channel] = convolve(img[:, :, channel], kernel, mode='constant', cval=0)
...
def blur():
    img = Image.open("static/img/img_now.jpg")
    img_arr = np.asarray(img, dtype=int)  # Ganti np.int dengan int
...
def histogram_rgb():
    ...
    # Simpan plot sebagai file sementara
    plot_path = 'temp_plot.png'
    plt.savefig(plot_path)
    plt.close()

    # Kirim file plot sebagai respons HTTP
...
def threshold(lower_thres, upper_thres):
    # Buka gambar dan buat array yang writable
    img = Image.open("static/img/img_now.jpg")
    img_arr = np.asarray(img)

    # Buat salinan untuk menjadikannya writable
    writable_img_arr = np.copy(img_arr)

    # Buat kondisi untuk threshold
    condition = np.logical_and(
        np.greater_equal(writable_img_arr, lower_thres),
        np.less_equal(writable_img_arr, upper_thres)
    )

    # Terapkan threshold
    writable_img_arr[condition] = 255
    writable_img_arr[~condition] = 0  # Contoh: set piksel di luar batas threshold menjadi 0
